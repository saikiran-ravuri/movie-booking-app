...
function Showdate({ date, onDateSelect }) {
  ...
  return (
    <div className="flex items-center gap-2.5 overflow-x-auto pb-2 pt-1 no-scrollbar max-w-full">
      {upcomingDates.map((item) => {
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center justify-center min-w-[72px] sm:min-w-[76px] px-3 py-2 rounded-2xl border transition-all cursor-pointer whitespace-nowrap shrink-0 shadow-2xs ${
              isSelected
                ? 'bg-slate-950 text-white border-slate-950 shadow-md scale-105 font-bold'
                : 'bg-white text-slate-700 border-slate-200/90 hover:border-slate-400 hover:bg-slate-50 font-medium'
            }`}
          >

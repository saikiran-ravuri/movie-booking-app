...
function Register({ isModal = false, onSwitchToLogin }) {
  ...
  const formContent = (
    <div className="w-full space-y-[clamp(0.75rem,1.5vw,1.25rem)] text-left">
      ...
      <form onSubmit={handleSubmit} className="space-y-[clamp(0.5rem,1vw,0.875rem)]">
        <div>
          <label className="block text-[clamp(0.65rem,0.8vw,0.75rem)] font-semibold text-slate-700 mb-1 text-left">
            Name
          </label>
          <input
            type="text"
